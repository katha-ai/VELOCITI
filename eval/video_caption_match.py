...
def vid_cap_match(dataloader, model, tokenizer, device, model_type, args):
    ...
    with torch.no_grad():
        ...
        for item in pb:
            ...
            ev1_frame_feats = get_video_features(
                model=model, frames=frames_ev1, device=device, model_type=model_type
            )
            ev5_frame_feats = get_video_features(
                model=model, frames=frames_ev5, device=device, model_type=model_type
            )

            ev1_cap_feats = get_cap_features(
                model=model,
                cap=pos_cap_ev1,
                tokenizer=tokenizer,
                device=device,
            ...
            ev5_cap_feats = get_cap_features(
                model=model,
                cap=pos_cap_ev5,
                tokenizer=tokenizer,
                device=device,
            ...
            gt_res = torch.logical_and(
                (v1_c1_score > v5_c1_score), (v5_c5_score > v1_c5_score)
            )
            gv_res = torch.logical_and(
                (v1_c1_score > v1_c5_score), (v5_c5_score > v5_c1_score)
            )
            ...
            ind_gt_cnt += (v1_c1_score > v5_c1_score).sum() + (
                v5_c5_score > v1_c5_score
            ).sum()
            ind_gv_cnt += (v1_c1_score > v1_c5_score).sum() + (
                v5_c5_score > v5_c1_score
            ).sum()
