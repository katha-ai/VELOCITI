    ...
    def __init__(self, config: CLIPVisionConfig, additional_vision_config=None):
        ...
        if self.if_use_temporal_embed:
            self.temporal_embedding = nn.Parameter(
                torch.zeros(1, self.temporal_size, self.embed_dim)
            )
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        embeds = torch.cat(
            [
                class_embeds,
                added_cls,
                patch_embeds.reshape(patch_embeds.shape[0], -1, patch_embeds.shape[-1]),
            ],
            dim=1,
        )
...
class CLIPAttention(nn.Module):
    ...
    def forward2(self, hidden_states, inputs_size):
        ...
        # in-frame attention:
        q = query_states[:, M:].reshape(
            -1, L, self.head_dim
        )  # [B*num_heads*N, L, head_dim]
        k = (
            key_states[:, :M].repeat(1, N, 1).reshape(-1, M, self.head_dim)
        )  # [B*num_heads*N, M, head_dim]
        k = torch.cat(
            [k, key_states[:, M:].reshape(-1, L, self.head_dim)], dim=1
        )  # [B*num_heads*N, M+L, head_dim]
        v = (
            value_states[:, :M].repeat(1, N, 1).reshape(-1, M, self.head_dim)
        )  # [B*num_heads*N, M, head_dim]
        v = torch.cat(
            [v, value_states[:, M:].reshape(-1, L, self.head_dim)], dim=1
        )  # [B*num_heads*N, M+L, head_dim]
...
class CLIPEncoderLayer(nn.Module):
    ...
    def forward(
        ...
        if isinstance(hidden_states, tuple):
            ...
            hidden_states = (
                self.layer_norm1(hidden_states[0]),
                self.layer_norm1(hidden_states[1]),
            )
...
class CLIPPreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        elif isinstance(module, CLIPVisionEmbeddings) or isinstance(
            module, CLIPVisionViPEmbeddings
        ):
            factor = self.config.initializer_factor
            nn.init.normal_(
...
class CLIPModel(CLIPPreTrainedModel):
    ...
    def __init__(self, config: CLIPConfig):
        ...
        self.vision_model = CLIPVisionTransformer(
            vision_config, additional_vision_config
        )
