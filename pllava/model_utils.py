...
def pllava_answer(
    ...
    # input ids minus the SOS token
    token_id_yes = processor.tokenizer("Yes")["input_ids"][1]
    token_id_no = processor.tokenizer("No")["input_ids"][1]
    ...
    with torch.no_grad():
        ...
        output_text = processor.batch_decode(
            output_token["sequences"],
            skip_special_tokens=True,
            clean_up_tokenization_spaces=False,
        )[0]

        if cal_entail:
            entailment_score, _, _ = get_score(
                output_token["scores"][0][0], token_id_yes, token_id_no
            )
    ...
    return (output_text, entailment_score)
