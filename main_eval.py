...
def get_all_metrics(model, tokenizer, transform, model_type, model_name, device):
    ...
    vc_match_metric, vc_match_df = vid_cap_match(
        dataloader=vc_match_loader,
        model=model,
        tokenizer=tokenizer,
        device=device,
    ...
    metric_dict["match_wino_t2v"] = vc_match_metric["video-caption-matching"]["wino"][
        "t2v"
    ]
    metric_dict["match_wino_v2t"] = vc_match_metric["video-caption-matching"]["wino"][
        "v2t"
    ]
    metric_dict["match_wino_group"] = vc_match_metric["video-caption-matching"]["wino"][
        "group"
    ]
    metric_dict["match_ind_t2v"] = vc_match_metric["video-caption-matching"]["ind"][
        "t2v"
    ]
    metric_dict["match_ind_v2t"] = vc_match_metric["video-caption-matching"]["ind"][
        "v2t"
    ]

    # control task
    print("\n<----- Control Task Evaluation ----->\n")
    control_dataset = controlDataset(data_dict=data_dict, transform=transform)

    control_dataloader = create_dataloader(dataset=control_dataset, args=args)

    control_metric_v2t, control_v2t_df = genericeval(
    ...
    control_metric_t2v, control_t2v_df = genericeval(
        dataloader=control_dataloader,
        model=model,
        tokenizer=tokenizer,
        task="t2v",
    ...
    agiden_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="ag_iden"
    )

    arg0en_dataloader = create_dataloader(dataset=agiden_dataset, args=args)

    arg0hn_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="ag_bind"
    )

    arg0hn_dataloader = create_dataloader(dataset=arg0hn_dataset, args=args)

    verb_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="action_bind"
    )

    verb_dataloader = create_dataloader(dataset=verb_dataset, args=args)

    manner_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="action_mod"
    )

    manner_dataloader = create_dataloader(dataset=manner_dataset, args=args)

    verb_cot_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="action_adv"
    )

    verb_cot_dataloader = create_dataloader(dataset=verb_cot_dataset, args=args)

    coref_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="coref"
    )

    coref_dataloader = create_dataloader(dataset=coref_dataset, args=args)

    seq_dataset = negDataset(
        data_dict=data_dict, transform=transform, neg_sampling="sequence"
    )

    seq_dataloader = create_dataloader(dataset=seq_dataset, args=args)

    print("\n<----- Agent-Identity-Test Task Evaluation ----->\n")
    ag_iden_metric, ag_iden_df = negeval(
        dataloader=arg0en_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
    ...
    ag_bind_metric, ag_bind_df = negeval(
        dataloader=arg0hn_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
    ...
    act_bind_metric, act_bind_df = negeval(
        dataloader=verb_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
    ...
    act_mod_metric, act_mod_df = negeval(
        dataloader=manner_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
    ...
    act_adv_metric, act_adv_df = negeval(
        dataloader=verb_cot_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
    ...
    coref_metric, coref_df = negeval(
        dataloader=coref_dataloader,
        model=model,
        tokenizer=tokenizer,
        device=device,
        model_type=model_type,
...
if __name__ == "__main__":
    ...
    if args.all:
        ...
        for model in models.keys():
            ...
            metric_dict, all_df = get_all_metrics(
                model,
                tokenizer,
                transform,
                model_type=model_type,
